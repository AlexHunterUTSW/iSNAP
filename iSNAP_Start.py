if __name__ == '__main__':
    ...
    if getattr(sys, 'frozen', False):
        # If the application is a Pyinstaller bundle
        application_path = join(dirname(sys.executable), '_internal')
    else:
        # If the application is a normal Python script
    ...
    class MainWindow(QWidget):
        ...
        def __init__(self):
            ...
            self.setWindowIcon(QIcon(join(application_path,'iSNAP_Icon.png')))
            ...
            self.toWorkerSave.connect(self.worker.save)
            ...
            self.worker.saveSignal.connect(self.saveEnd)
            
            self.mainbox = QHBoxLayout()
            self.subbox = QVBoxLayout()

            # Assign pages
            self.page0 = SetModality(application_path)
            ...
            self.labelLoading = QLabel('Loading... ')

            self.save_btn = QPushButton('Save .h5ad of Analysis')
            self.save_btn.clicked.connect(self.saveStart)
            ...
            self.btn_box.addWidget(self.labelLoading)
        ...
        def saveStart(self):
            self.labelLoading.setText('Saving... ')
            ...
            isSub = self.stackedWidget.currentIndex() == 14
            self.toWorkerSave.emit(isSub)
        ...
        def nextpgStart(self, currentPage = None, h5adDetected=False):
            try:
                ...
                # Page 10 Logic: Subcluster, Get NPC and Integration method, Create UMAP
                    ...
                    paramDict['intMethodSub'] = self.page10[0].inInt.currentText()
        ...
        def closeEvent(self, event):
            ...
            if self.thread and self.thread.isRunning():
                print("Main window is closing. Stopping thread...")
                self.worker.stop()
                self.thread.wait()
            event.accept()
    ...
    class MainWorker(QObject):
        ...
        def processData(self, inParamDict, currentPage):
            ...
            if 'h5adDetected' in inParamDict.keys():
                if currentPage == 8:
                    ...
                    delattr(self, 'adatasRaw')

            if currentPage == 14:
                try:
                    print('')
                    leidenToCellType(self.adatasSub, self.paramDict['cellTypesSub'])
                    ...
                    newcelltype = self.adatasSub.obs['cell_type']
                    oldcelltype = self.adatasWhole[self.adatasWhole.obs['cell_type']!=self.paramDict['chosenType']].obs['cell_type']

                    combcelltype = concat([newcelltype, oldcelltype])    

                    self.adatasWhole.obs['cell_type'] = combcelltype.astype('category')
                    
                    size = 240000/self.adatasWhole.n_obs
                    figUMAP = pltumap(self.adatasWhole, show=False, color="cell_type", frameon = False, palette=default_102, size=size, title='Cell Type', return_fig=True)
                    ...
                    inputSetPage['cellTypes'] = self.adatasWhole.obs['cell_type'].cat.categories
                ...
                try: 
                    ...
                    degResults = self.adatasSub.uns['DEG']
                    ...
                    for i in range(len(self.clustersSub)):
                        ...
                        figUMAP = specificClusterUMAP(self.adatasSub, self.tempTypes10[self.clustersSub[i]], key='tempType', suffix=suffix)
                        figUMAP.set_constrained_layout(True)
            ...
            # Page 12 Logic: Subcluster, select Leiden Resolution
                ...
                try:
                    ...
                    deg(self.adatasSub, self.paramDict['DEGMethodSub'])

                    suffix = self.paramDict['chosenType']

                    degtoCSV(self.adatasSub, self.paramDict['output_path'], self.paramDict['DEGMethodSub'], suffix=suffix)

                    # Set up Dot Plot
                    dendrogram(self.adatasSub, groupby='leiden', n_pcs=self.paramDict['NPCSub'])
                    figDEG = rank_genes_groups_dotplot(self.adatasSub, n_genes=5, key='DEG', groupby="leiden", show=False, return_fig=True)
            ...
            # page 11 Logic: Subcluster, get test Leiden resolutions, Plot with Leiden
                ...
                try:
                    ...
                    for i in range(len(self.paramDict['resListSub'])):
                        self.adatasSub.obs['temp_leiden'] = self.leidenListSub[i][0]

                        silhouetteScore.append(silhouetteLeiden(self.adatasSub, column='temp_leiden'))

                        size = 240000/self.adatasSub.n_obs
                        figUMAPList.append(pltumap(self.adatasSub, show=False, color='temp_leiden', frameon = False, palette=default_102, title=f'Resolution {self.paramDict["resListSub"][i]}, {suffix}', size=size, return_fig=True))
                        ...
                        self.adatasSub.obs.drop(columns='temp_leiden', inplace=True)
                ...
                try:
                    with open(join(self.paramDict['output_path'], 'iSNAP User Log.txt'), 'a+') as log:
                        ...
                        lines = [
                            ...
                            f'Integration Method = {self.paramDict["intMethodSub"]}\n\n',
                        ]
                        log.writelines(lines)

                    integration(self.adatasSub, self.paramDict["NPCSub"], self.paramDict["nNeighSub"], self.paramDict["intMethodSub"], self.paramDict['seed'])
                    suffix = self.paramDict['chosenType']

                    size = 240000/self.adatasSub.n_obs
                    figUMAP = pltumap(self.adatasSub, show=False, frameon=False, color='sample', size=size, title=f'Samples, {suffix}', return_fig=True) 
                    figUMAP.set_constrained_layout(True)
            ...
            # Page 9 Logic: Go to Post Analysis
                ...
                try:
                    ...
                    types = list(self.adatasWhole.obs['cell_type'].cat.categories)
                    ...
                    for i in range(len(samples)-1):
                        ...
                        for j in range(len(types)-1):
                            sampleTypeMtx[j, i] = self.adatasWhole[(self.adatasWhole.obs['sample']==samples[i]) & (self.adatasWhole.obs['cell_type']==types[j])].n_obs
                    
                    for i in range(len(types)-1):
                        sampleTypeMtx[i, len(samples)-1] = self.adatasWhole[self.adatasWhole.obs['cell_type']==types[i]].n_obs
            ...
            # Page 8 Logic: Annotate Cluster, Go to Cell type page
                ...
                try:
                    if not self.paramDict['h5adImport']:
                        leidenToCellType(self.adatasWhole, self.paramDict['cellTypes'])
                        with open(join(self.paramDict['output_path'], 'iSNAP User Log.txt'), 'a+') as log:
            ...
            # Page 7 Logic: Differentially expressed genes Heatmap, Annotation Set up
                ...
                try: 
                    ...
                    degResults = self.adatasWhole.uns['DEG']
                    ...
                    for i in range(len(self.clusters)):
                        ...
                        figUMAP = specificClusterUMAP(self.adatasWhole, self.tempTypes8[self.clusters[i]], key='tempType', suffix=suffix)
                        figUMAP.set_constrained_layout(True)
            ...
            # Page 6 Logic: Differentially expressed genes
                ...
                try:
                    ...
                    deg(self.adatasWhole, self.paramDict['DEGMethod'])

                    degtoCSV(self.adatasWhole, self.paramDict['output_path'], self.paramDict['DEGMethod'])

                    suffix = 'All'

                    # Set up Dot Plot
                    dendrogram(self.adatasWhole, groupby='leiden', n_pcs=self.paramDict['NPC'])
                    figDEG = rank_genes_groups_dotplot(self.adatasWhole, n_genes=5, key='DEG', groupby="leiden", show=False, return_fig=True)
                    # *** Make own for clean labeling
            ...
            # Page 5 Logic: Input Resolution, Clustering
                ...
                try:
                    ...
                    for i in range(len(self.paramDict['resList'])):
                        self.adatasWhole.obs['temp_leiden'] = self.leidenList[i][0]

                        silhouetteScore.append(silhouetteLeiden(self.adatasWhole, column='temp_leiden'))

                        size = 240000/self.adatasWhole.n_obs
                        figUMAPList.append(pltumap(self.adatasWhole, show=False, color='temp_leiden', frameon = False, palette=default_102, title=f'Resolution {self.paramDict["resList"][i]}, {suffix}', return_fig=True))
                        figUMAPList[i].set_constrained_layout(True)
                        figUMAPList[i].savefig(join(outpath, f'Leiden Resolution {self.paramDict["resList"][i]}.png'))

                        self.adatasWhole.obs.drop(columns='temp_leiden', inplace=True)
            ...
            # Page 4 Logic: Inputs PC and Integration, Integration & UMAP
                ...
                try:
                    ...
                    integration(self.adatasWhole, self.paramDict['NPC'], self.paramDict['nNeigh'], self.paramDict['intMethod'], self.paramDict['seed'])

                    suffix = 'All'

                    size = 240000/self.adatasWhole.n_obs
                    figUMAP = pltumap(self.adatasWhole, show=False, frameon=False, color='sample', size=size, title=f'Samples, {suffix}', return_fig=True) 
                    figUMAP.set_constrained_layout(True)
                    # *** Make own for clean labelling
            ...
            # Page 3 Logic: Normalization
                ...
                try:
                    ...
                    # *** Could Save adatasHVG as h5ad and then re-read when using back_btn or clicking the progress tab. Would need to create a back function for MainWorker
                    ...
                    var = self.adatasWhole.uns['pca']['variance_ratio']
            ...
            # Page 2 Logic: Quality Control
                ...
                try:
                    ...
                    # *** Could Save adatasRaw as h5ad and then re-read when using back_btn or clicking the progress tab. Would need to create a back function for MainWorker
                    self.adatasBeforeHVG = quality_control(
                        self.adatasRaw.copy(), 
                    ...
                    filteredtoCSV(self.adatasRaw, self.adatasBeforeHVG, self.paramDict['output_path'])
            ...
            # Page 1 Logic: Input Folders & Read
                ...
                try:
                    ...
                    set_global_seed(self.paramDict['seed'])

                    # Read data and save as  adatasRaw
                    ...
                    if self.paramDict['modality'] == 'Single .h5ad file':
                        if not 'cell_type' in self.adatasRaw.obs.columns and 'celltype' in self.adatasRaw.obs.columns:
                            self.adatasRaw.obs['cell_type'] = self.adatasRaw.obs['celltype'].copy()
                            self.adatasRaw.obs.drop(columns = 'celltype', inplace=True)

                        if 'cell_type' in self.adatasRaw.obs.columns:
        ...
        def save(self, isSub):
            try:
                if isSub:
                    ...
                    save_adatas(self.adatasWhole, self.paramDict['output_path'], suffix='All')
        ...
        def toDGE(self, bySamples):
            ...
            try:
                if bySamples:
                    mainItems = self.adatasWhole.obs['cell_type'].cat.categories.tolist()
                    groupItems = self.adatasWhole.obs['sample'].cat.categories.tolist()
                else:
                    mainItems = self.adatasWhole.obs['sample'].cat.categories.tolist()
                    groupItems = self.adatasWhole.obs['cell_type'].cat.categories.tolist()
        ...
        def toVolcano(self, mainList, groupAList, groupBList, bySamples, DGEMethod, nTopGenes, listGenes, pvalThresh, logFCThresh):
                try:
                    ...
                    if bySamples:
                        adatas = self.adatasWhole[self.adatasWhole.obs['cell_type'].isin(mainList)].copy()
                        adatas = adatas[(adatas.obs['sample'].isin(groupAList)) | (adatas.obs['sample'].isin(groupBList))]
                        mapDGE = {}
                        ...
                        adatas = self.adatasWhole[self.adatasWhole.obs['sample'].isin(mainList)]
                        adatas = adatas[(adatas.obs['cell_type'].isin(groupAList)) | (adatas.obs['cell_type'].isin(groupBList))].copy()
                        ...
                        adatas.obs['groupDGE'] = adatas.obs['cell_type'].copy().map(mapDGE)
        ...
        def toSubCluster(self, chosenType):
            ...
            try:
                ...
                self.adatasSub = self.adatasWhole[self.adatasWhole.obs['cell_type'] == chosenType].copy()
                ...
                figPCA = pcaVarianceRatio(self.adatasSub, n_pcs=50)
                figPCA.savefig(join(outpath, f'PCAVarianceRatio_{suffix}.png'))

                # Find optimal PCs by using maximum distance from endpoint line
                var = self.adatasSub.uns['pca']['variance_ratio']
                pcList = range(1, len(var)+1)
        ...
        def combTypes(self, type1, type2, newName):
            celltypes = self.adatasWhole.obs['cell_type'].cat.categories
            typemap = {}
            ...
            self.adatasWhole.obs['cell_type'] = self.adatasWhole.obs['cell_type'].copy().map(typemap).astype('category')
            size = 240000/self.adatasWhole.n_obs
            figUMAP = pltumap(self.adatasWhole, show=False, color="cell_type", frameon = False, palette=default_102, size=size, title='Cell Type', return_fig=True)
            figUMAP.set_constrained_layout(True)
        ...
        def toAnnotate(self, groupIndex, newName, isMain):
            try:
                if isMain:
                    ...
                    # Specific UMAP
                    figUMAPSpecific = specificClusterUMAP(self.adatasWhole, self.tempTypes8[self.clusters[groupIndex]], key='tempType', suffix=suffix)
                    ...
                    # Specific UMAP
                    figUMAPSpecific = specificClusterUMAP(self.adatasSub, self.tempTypes10[self.clustersSub[groupIndex]], key='tempType', suffix=suffix)
                    figUMAPSpecific.set_constrained_layout(True)
